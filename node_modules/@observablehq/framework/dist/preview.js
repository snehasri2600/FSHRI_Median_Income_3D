...
class PreviewServer {
  ...
  static async start({ verbose = true, hostname, port, open, ...options }) {
    ...
    if (verbose) {
      console.log(`${green(bold("Observable Framework"))} ${faint(`v${"1.13.4"}`)}`);
...
function handleWatch(socket, req, configPromise) {
  ...
  async function hello({ path: initialPath, hash: initialHash }) {
    ...
    if (resolvers.hash === initialHash) send2({ type: "welcome" });
    else return void send2({ type: "reload" });
    hash = resolvers.hash;
    html = getHtml(page, resolvers);
    code = getCode(page, resolvers);
    files = getFiles(resolvers);
    ...
    if (config.watchPath) configWatcher = watch(config.watchPath, () => send2({ type: "reload" }));
  }
