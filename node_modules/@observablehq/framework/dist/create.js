...
async function create(effects = defaultEffects) {
  const { clack } = effects;
  clack.intro(`${inverse(" observable create ")} ${faint(`v${"1.13.4"}`)}`);
      ...
      installing: async ({ results: { rootPath, appTitle, includeSampleFiles, packageManager, initializeGit } }) => {
        ...
        await recursiveCopyTemplate(
          ...
          rootPath,
          {
            ...
            frameworkVersionString: JSON.stringify(`^${"1.13.4"}`)
          },
          effects
        );
        if (packageManager) {
...
`
        ...
        if (spinning) s.stop("Installed! \u{1F389}");
        const instructions = [];
...
function validateRootPath(rootPath, defaultError) {
  if (rootPath === "") return defaultError;
