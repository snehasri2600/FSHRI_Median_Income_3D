...
function transpileSql(content, { id, display } = {}) {
  if (id !== void 0 && !isValidBinding(id)) throw new SyntaxError(`invalid binding: ${id}`);
...
function isValidBinding(input) {
  try {
    ...
    if (token.start !== 0 || token.end !== input.length) return false;
    if (parser.type !== tokTypes.eof) return false;
    return true;
  } catch {
    return false;
  }
