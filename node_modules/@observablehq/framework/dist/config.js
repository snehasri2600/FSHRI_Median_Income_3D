...
async function readConfig(configPath, root) {
  if (configPath === void 0) configPath = await resolveDefaultConfig(root);
  if (configPath === void 0) return normalizeConfig(void 0, root);
...
async function resolveDefaultConfig(root) {
  ...
  if (existsSync(tsPath)) return await import("tsx/esm"), tsPath;
}
...
function normalizeConfig(spec = {}, defaultRoot, watchPath) {
  ...
  if (pages === void 0) Object.defineProperty(config, "pages", { get: () => readPages(root, md) });
  if (sidebar === void 0) Object.defineProperty(config, "sidebar", { get: () => config.pages.length > 0 });
...
function findDefaultRoot(defaultRoot) {
  if (defaultRoot !== void 0) return defaultRoot;
...
function normalizeDuckDB(spec) {
  ...
  if (extspec.json === void 0) extspec = { ...extspec, json: false };
  if (extspec.parquet === void 0) extspec = { ...extspec, parquet: false };
