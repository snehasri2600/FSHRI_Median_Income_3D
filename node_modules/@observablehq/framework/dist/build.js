  ...
  for await (const path of config.paths()) {
    ...
    if (path.endsWith(".js")) {
      ...
      if (module) {
        ...
        for (const f of resolvers2.files) addFile(path, f);
        for (const i of resolvers2.localImports) addLocalImport(path, i);
        for (const i of resolvers2.globalImports) addGlobalImport(path, resolvers2.resolveImport(i));
        for (const s of resolvers2.stylesheets) addStylesheet(path, s);
